...
export default class Component extends React.Component {
  ...
  _onChange = (state, e) => {
    if (regex.regExToCheckEmail.test(this.state.email) && this.state.password.length > 1) {
  ...
  _signin = async () => {
    ...
    try {
      ...
      if (result.code === 200) {
        ...
        ToastAndroid.show(`Failed to login, ${result.message}`, ToastAndroid.LONG);
      }
    } catch (err) {
        ...
        <View style={styles.container}>
          <ImageBackground source={images.bgimageful} style={styles.bgimage}>
            <View style={styles.mrg60}>
              ...
              <TouchableHighlight style={styles.donthaveacc} onPress={this._toLogin}>
                <Text style={styles.txtPlaceholder}>{I18n.t('donthaveacc')}</Text>
              </TouchableHighlight>
            </View>
            {this.state.off ? null : (
              <TouchableHighlight style={styles.viewTxtReg} onPress={this._signin}>
